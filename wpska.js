...
function _load(files, callback) {
	...
	for(var i in files) {
		...
		if (! regex.test(_testloaded)) loads.push(files[i][1]);
	}
	callback = (typeof callback=="function")? callback: function() {};
	head.load(loads, callback);
}
...
function _dir(path) {
	return (__dir+path).replace(/^\/|\/$/g, '');
}


$$ = function(selector, loads, callback) {var $=jQuery; if ($(selector).length>0) { head.load(loads); }};
